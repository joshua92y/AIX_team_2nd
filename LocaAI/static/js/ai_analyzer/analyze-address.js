...
function openAddressSearch() {
  ...
  new daum.Postcode({
    ...
    onmousedown: function() {
      console.log('🔍 [DEBUG] 주소 검색 시작 - 서울특별시만 검색 가능');
    },
    onsearch: function(data) {
      console.log('🔍 [DEBUG] 검색어:', data.q);
      // 서울이 아닌 지역명이 포함된 검색어인 경우 경고
      const nonSeoulKeywords = [
        '부산', '대구', '인천', '광주', '대전', '울산', '세종',
        '경기', '강원', '충북', '충남', '전북', '전남', '경북', '경남', '제주'
      ];
      
      const searchQuery = data.q.toLowerCase();
      const hasNonSeoulKeyword = nonSeoulKeywords.some(keyword => 
        searchQuery.includes(keyword)
      );
      
      if (hasNonSeoulKeyword) {
    ...
    oncomplete: function(data) {
      ...
      if (!validateSeoulAddress(finalAddress)) {
        ...
        // 더 상세하고 사용자 친화적인 경고 메시지
        alert('🚫 서비스 지역 제한 안내\n\n' +
              '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n' +
              '현재 서비스는 서울특별시 지역만 지원됩니다.\n\n' +
              '✅ 지원 지역: 서울특별시 25개 구\n' +
              '   (강남구, 강동구, 강북구, 강서구, 관악구, 광진구 등)\n\n' +
              '❌ 미지원 지역: 경기도, 인천시, 기타 광역시/도\n\n' +
              '서울특별시 내의 주소를 다시 검색해주세요.\n' +
              '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    autoMapping: true,
    maxSuggestItems: 5,
    // 사용자에게 서울 지역만 검색 가능함을 알림
    onready: function() {
      console.log('🔍 [DEBUG] 주소 검색 준비 완료 - 서울특별시 지역만 지원');
      
      // 검색창에 힌트 추가 (가능한 경우)
      const searchInput = document.querySelector('#addressSearchContainer input');
      if (searchInput) {
        searchInput.placeholder = '서울특별시 내 주소를 입력하세요 (예: 강남구 테헤란로)';
      }
    }
...
// 서울특별시 주소인지 검증하는 함수 (강화된 버전)
function validateSeoulAddress(address) {
  console.log('🔍 [DEBUG] 주소 검증 중:', address);
  ...
  // 먼저 서울이 아닌 지역이 포함되어 있는지 확인 (더 엄격한 검증)
  const hasNonSeoulKeyword = nonSeoulKeywords.some(keyword => address.includes(keyword));
  if (hasNonSeoulKeyword) {
    console.log('❌ [DEBUG] 서울 이외 지역 키워드 발견:', address);
    return false;
  }
  
  // 서울 관련 키워드가 포함되어 있는지 확인
  const hasSeoulKeyword = seoulKeywords.some(keyword => address.includes(keyword));
  console.log('✅ [DEBUG] 서울 키워드 확인:', hasSeoulKeyword);
  
  return hasSeoulKeyword;
}
