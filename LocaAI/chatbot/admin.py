...
class ChatMemoryInline(admin.TabularInline):
    ...
    def content_preview(self, obj):
        if obj.content:
            ...
            return format_html('<span title="{}">{}</span>', content_str, content_str + '...' if len(content_str) > 100 else content_str)
        return '-'
...
class ChatLogInline(admin.StackedInline):
    ...
    def log_preview(self, obj):
        if obj.log:
            ...
            return format_html('<strong>{}개의 메시지</strong><br><small>마지막 업데이트: {}</small>', 
                             count, obj.updated_at.strftime('%Y-%m-%d %H:%M'))
        return '로그 없음'
...
class ChatSessionAdmin(admin.ModelAdmin):
    list_display = ('session_id', 'user_link', 'title', 'memory_count', 'collection_count', 'session_duration', 'created_at', 'lastload_at')
        ...
        try:
            ...
            for url_name in possible_urls:
                try:
                    ...
                    return format_html('<a href="{}" style="color: #0066cc; text-decoration: none;">{} <span style="color: #666; font-size: 11px;">({})</span></a>', 
                                     url, obj.user.username, obj.user.role)
                except NoReverseMatch:
                    continue
            
            # 모든 URL이 실패하면 강조된 텍스트로 표시
            return format_html('<strong style="color: #0066cc;">{}</strong> <span style="color: #666; font-size: 11px;">({})</span>', 
                             obj.user.username, obj.user.role)
    ...
    def session_duration(self, obj):
        if obj.lastload_at and obj.created_at:
            ...
            if hours < 1:
                ...
                return format_html('<span style="color: #0066cc;">{}시간</span>', hours_formatted)
            else:
                days_formatted = f"{hours / 24:.1f}"
                return format_html('<span style="color: #cc6600;">{}일</span>', days_formatted)
        return '-'
    ...
    def session_info(self, obj):
        ...
        return format_html(
            ...
            '• 컬렉션 메모리: {} 개<br>'
            '• 채팅 로그: {} 개<br>'
            '</div>', memories, collections, log_count
        )
        ...
        for session in queryset:
            ...
            for memory_type in ['question', 'answer', 'summary']:
                ...
                if memories.count() > 30:
                    ...
                    count = ChatMemory.objects.filter(id__in=[m.id for m in to_delete]).count()
                    ChatMemory.objects.filter(id__in=[m.id for m in to_delete]).delete()
                    total_deleted += count
        
        self.message_user(request, f"총 {total_deleted}개의 오래된 메모리가 정리되었습니다.")
    cleanup_old_memories.short_description = "오래된 메모리 정리"


@admin.register(ChatMemory)
class ChatMemoryAdmin(admin.ModelAdmin):
    list_display = ('id', 'session_link', 'memory_type_badge', 'role_badge', 'content_preview', 'created_at')
    ...
    fieldsets = (
        ('기본 정보', {
            'fields': ('session', 'memory_type', 'role')
        }),
        ('내용', {
    ...
    def memory_type_badge(self, obj):
        ...
        return format_html(
            ...
            color, obj.get_memory_type_display()
        )
    ...
    def content_preview(self, obj):
        if obj.content:
            ...
            return format_html('<span title="{}">{}</span>', 
                             str(obj.content), content_str + '...' if len(content_str) > 80 else content_str)
        return '-'
    ...
    def content_formatted(self, obj):
        if obj.content:
            ...
            try:
                ...
                return format_html('<pre style="background: #f8f9fa; padding: 10px; border-radius: 5px; max-height: 300px; overflow-y: auto;">{}</pre>', formatted)
            except:
                return format_html('<pre style="background: #f8f9fa; padding: 10px; border-radius: 5px;">{}</pre>', str(obj.content))
        return '-'
...
class PromptAdmin(admin.ModelAdmin):
    ...
    def tag_list(self, obj):
        if obj.tag:
            ...
            for tag in tags:
                tag = tag.strip()
                tag_html.append(f'<span style="background: #e9ecef; padding: 1px 6px; border-radius: 3px; font-size: 10px; margin-right: 2px;">{tag}</span>')
    ...
    def content_preview(self, obj):
        ...
        return format_html('<span title="{}">{}</span>', 
                         obj.content, preview + '...' if len(obj.content) > 100 else preview)
    content_preview.short_description = '내용 미리보기'
...
class CollectionMemoryAdmin(admin.ModelAdmin):
    list_display = ('id', 'session_link', 'collection_name_badge', 'prompt_link', 'response_preview', 'doc_count', 'created_at')
    ...
    fieldsets = (
        ('기본 정보', {
            'fields': ('session', 'collection_name', 'prompt')
        }),
        ('문서 정보', {
            'fields': ('documents_info', 'retrieved_documents_content', 'retrieved_documents_meta'),
            'classes': ('collapse',)
        }),
        ('응답', {
    ...
    def response_preview(self, obj):
        if obj.llm_response:
            ...
            return format_html('<span title="{}">{}</span>', 
                             obj.llm_response, preview + '...' if len(obj.llm_response) > 80 else preview)
        return '-'
    response_preview.short_description = 'LLM 응답'
    
    def doc_count(self, obj):
        try:
            content_count = len(obj.retrieved_documents_content) if obj.retrieved_documents_content else 0
            meta_count = len(obj.retrieved_documents_meta) if obj.retrieved_documents_meta else 0
            return format_html('<span style="color: #007bff;">📄 {}/📊 {}</span>', content_count, meta_count)
        except:
            return '-'
    ...
    def response_formatted(self, obj):
        if obj.llm_response:
            return format_html('<div style="background: #f8f9fa; padding: 10px; border-radius: 5px; max-height: 300px; overflow-y: auto; white-space: pre-wrap;">{}</div>', obj.llm_response)
        return '-'
...
class ChatLogAdmin(admin.ModelAdmin):
    list_display = ('session_link', 'message_count', 'last_message_preview', 'updated_at')
    ...
    def last_message_preview(self, obj):
        if obj.log and isinstance(obj.log, list) and obj.log:
            ...
            return format_html(
                ...
                role_color, role, content, '...' if len(content) == 50 else ''
            )
        return '-'
    ...
    def log_formatted(self, obj):
        if obj.log and isinstance(obj.log, list):
            ...
            if len(obj.log) > 5:
                html_parts.insert(0, f'<p style="color: #6c757d; font-style: italic;">... {len(obj.log) - 5}개 더 많은 메시지 (최근 5개만 표시)</p>')
                
            return format_html('<div style="max-height: 400px; overflow-y: auto;">{}</div>', ''.join(html_parts))
        return '-'
    ...
    def log_stats(self, obj):
        if obj.log and isinstance(obj.log, list):
            ...
            assistant_count = sum(1 for msg in obj.log if msg.get('role') == 'assistant')
            ...
            return format_html(
                ...
                '• 사용자 메시지: {} 개<br>'
                '• 어시스턴트 메시지: {} 개<br>'
                '</div>', total, user_count, assistant_count
            )
        return '-'
