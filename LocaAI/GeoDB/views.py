...
def get_detailed_statistics():
    ...
    try:
        # 상점 업종별 분포
        store_types = StorePoint.objects.exclude(
            Q(uptaenm__isnull=True) | Q(uptaenm='')
        ).values('uptaenm').annotate(count=Count('uptaenm')).order_by('-count')[:10]
        
        # 학교 유형별 분포
        school_types = School.objects.exclude(
            Q(school_type__isnull=True) | Q(school_type='')
        ).values('school_type').annotate(count=Count('school_type')).order_by('-count')
        
        # 공공건물 분류별 분포
        building_types = PublicBuilding.objects.exclude(
            Q(lclas_cl__isnull=True) | Q(lclas_cl='')
        ).values('lclas_cl').annotate(count=Count('lclas_cl')).order_by('-count')[:10]
